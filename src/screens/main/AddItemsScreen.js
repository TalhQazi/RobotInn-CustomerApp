...
const AddItemsScreen = ({ navigation, route }) => {
  ...
  // Ensure currentItems have category metadata attached
  const enrichedCurrentItems = currentItems.map(item => {
    if (item.categoryId && item.categoryName && item.category) return item;
    const cat = CategoryDetectionService.detectCategory(item.text || item.name || '');
    const finalCatName = (cat.categoryName && cat.categoryName !== 'Other') ? cat.categoryName : 'Food';
    const finalCatId = (cat.categoryId && cat.categoryId !== 'other') ? cat.categoryId : 'food';
    return {
      ...item,
      category: finalCatName,
      categoryId: finalCatId,
      categoryName: finalCatName,
      categoryIcon: cat.categoryIcon || 'restaurant-outline',
      categoryDetected: cat.categoryDetected,
    };
  });

  const [selectedItems, setSelectedItems] = useState(enrichedCurrentItems);
  const [searchQuery, setSearchQuery] = useState('');
  const [customItem, setCustomItem] = useState('');

  // Add item from product list
  const addProductItem = (product) => {
    const cat = CategoryDetectionService.detectCategory(product.name);
    const finalCatName = (cat.categoryName && cat.categoryName !== 'Other') ? cat.categoryName : 'Food';
    const finalCatId = (cat.categoryId && cat.categoryId !== 'other') ? cat.categoryId : 'food';
    const newItem = {
  ...
  const addCustomItem = () => {
    ...
    if (trimmed) {
      const cat = CategoryDetectionService.detectCategory(trimmed);
      const finalCatName = (cat.categoryName && cat.categoryName !== 'Other') ? cat.categoryName : 'Food';
      const finalCatId = (cat.categoryId && cat.categoryId !== 'other') ? cat.categoryId : 'food';
      const newItem = {
        id: Date.now().toString() + Math.random(),
