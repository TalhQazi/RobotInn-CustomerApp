...
function mapApiOrderToView(o) {
  ...
  return {
    ...
    originalEstimate: o.originalEstimate || o.estimatedPrice || o.estimatedSubtotal || null,
    billAttachments: o.billAttachments || o.bill?.attachments || (o.bill?.receiptImageUrl ? [{ fileUrl: o.bill.receiptImageUrl, fileName: 'Store Receipt' }] : []),
  };
}
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Card style={styles.orderCard}>
          ...
          {/* Price Adjustment / Variance Callout Banner for Customer */}
          {(order.rawStatus === 'ADJUSTMENT_PENDING' || (order.originalEstimate && order.total > order.originalEstimate)) && (
            <View style={{ backgroundColor: '#FFF5F5', borderColor: '#FEB2B2', borderWidth: 1, borderRadius: 12, padding: 14, marginBottom: 16 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                <Ionicons name="warning-outline" size={20} color="#E53E3E" style={{ marginRight: 6 }} />
                <Text style={{ fontSize: 14, fontWeight: '700', color: '#C53030' }}>
                  Purchase Price Update
                </Text>
              </View>
              <Text style={{ fontSize: 13, color: '#4A5568', lineHeight: 18, marginBottom: 8 }}>
                The actual store purchase price (Rs. {parseFloat(order.total || 0).toFixed(0)}) is higher than your original estimate (Rs. {parseFloat(order.originalEstimate || 0).toFixed(0)}). Difference: +Rs. {(parseFloat(order.total || 0) - parseFloat(order.originalEstimate || 0)).toFixed(0)}.
              </Text>
              <TouchableOpacity
                style={{ backgroundColor: '#2EC4B6', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, alignSelf: 'flex-start' }}
                onPress={() => {
                  navigation.navigate('Chat', {
                    participantId: 'support_admin',
                    contactName: 'Admin Support',
                    orderCode: order.orderId,
                  });
                }}
              >
                <Text style={{ color: '#fff', fontSize: 12, fontWeight: '700' }}>💬 Open Support Chat</Text>
              </TouchableOpacity>
            </View>
          )}

          {/* Uploaded Bill Attachments Summary */}
          {Array.isArray(order.billAttachments) && order.billAttachments.length > 0 && (
            <View style={{ marginBottom: 16 }}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#2D3748', marginBottom: 8 }}>
                Store Purchase Receipts ({order.billAttachments.length})
              </Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {order.billAttachments.map((att, idx) => (
                  <View key={att.id || idx} style={{ width: 100, marginRight: 10, backgroundColor: '#F7FAFC', borderRadius: 8, padding: 6, borderWidth: 1, borderColor: '#E2E8F0' }}>
                    <Image source={{ uri: att.fileUrl || att.url }} style={{ width: 88, height: 88, borderRadius: 6, backgroundColor: '#EDF2F7' }} resizeMode="cover" />
                    <Text style={{ fontSize: 11, fontWeight: '600', color: '#4A5568', marginTop: 4 }} numberOfLines={1}>
                      Bill #{idx + 1}
                    </Text>
                    {Boolean(att.notes) && (
                      <Text style={{ fontSize: 10, color: '#718096' }} numberOfLines={1}>
                        {att.notes}
                      </Text>
                    )}
                  </View>
