...
const MyAddressesScreen = ({ navigation }) => {
  ...
  const handleAddAddress = async () => {
    ...
    try {
      ...
      // Local storage fallback so manual address entry is never lost
      const fallbackAddress = {
        id: `ADR-${Date.now()}`,
        addressId: `ADR-${Date.now()}`,
        ...payload,
      };
      const updatedAddresses = sortAddressesNewestFirst([fallbackAddress, ...addresses]);
      await saveAddresses(updatedAddresses);
      resetForm();
      setShowAddModal(false);
    } finally {
