...
const HERO_CARDS = [
  ...
  { id: '7', title: 'Baskin Robbins', subtitle: 'Ice Cream Delights', logoAsset: BRAND_LOGOS['baskin'], rating: '4.7', deliveryTime: '15-20', deliveryType: 'Free' },
];
...
const newOrderItem = (text = '') => {
  ...
  const finalCategoryName = (detectedObj?.categoryName && detectedObj?.categoryName !== 'Other')
    ? detectedObj.categoryName
    : 'Food';
  const finalCategoryId = (detectedObj?.categoryId && detectedObj?.categoryId !== 'other')
    ? detectedObj.categoryId
    : 'food';
  ...
  return {
    ...
    categoryIcon: detectedObj?.categoryIcon || 'restaurant-outline',
...
const KNOWN_BRAND_STORES = [
  // Fast Food & Restaurant Brands
  'kfc', 'mcdonald', 'pizza hut', 'subway', 'burger king', 'domino', 'starbucks',
  'dunkin', 'baskin', 'hardee', 'popeyes', 'tim hortons', 'papa john', 'nando',
  'cinnabon', 'texas chicken', 'gloria jean', 'costa coffee', 'krispy kreme',
  'taco bell', 'wendy', 'baskin robbins', 'dunkin donuts', 'cheezious', 'howdy', 'savour',
  // Pharmacy Brands
  'shaheen', 'watson', 'd.watson', 'd watson', 'shifa', 'servaid', 'metro pharmacy', 'city pharmacy',
  // Grocery & Mart Brands
  'savemart', 'save mart', 'imtiaz', 'punjab cash', 'greenvalley', 'carrefour', 'al-fatah', 'hyperstar',
  // Bakery Brands
  'tehzeeb', 'rahat', 'kitchen cuisine', 'layered',
  // Meat Brands
  'meat one', 'kausar'
];

const isInternationalBrandStore = (store) => {
  if (!store) return false;
  if (typeof store === 'object') {
    if (store.isInternational === true || store.isInternationalBrand === true || store.brandType === 'international') {
      return true;
    }
  }
  const name = (typeof store === 'string' ? store : store.name || '').toLowerCase();
  return KNOWN_BRAND_STORES.some(brand => name.includes(brand));
};
...
const getBrandLogo = (storeName) => {
  ...
  if (name.includes('burger king')) return BRAND_LOGOS['burger king'];
  if (name.includes('pizza hut')) return BRAND_LOGOS['pizza hut'];
  if (name.includes('kfc')) return BRAND_LOGOS['kfc'];
  if (name.includes('mcdonald')) return BRAND_LOGOS['mcdonald'];
...
const DashboardScreen = ({ navigation, route }) => {
  ...
  const [storePicker, setStorePicker] = useState({ visible: false, itemId: null, category: null });
  ...
  // ── Fetch areas (Real-time Firestore listener) ──────────────────────────────
  useEffect(() => {
    let unsubscribe = null;
    const applyAreas = (data) => {
    ...
    areasAPI.getAll().then(response => {
      if (response.success && response.data) applyAreas(response.data);
    }).catch(err => {
      console.log('❌ Areas initial fetch error:', err.message);
      setAreasLoading(false);
    });

    unsubscribe = areasAPI.subscribe(
      list => {
        applyAreas(list);
      },
      err => {
        console.warn('❌ Areas live subscription error:', err);
      }
    );
  ...
  const currentPickerCategory = storePicker.category || currentPickerItem?.category || activeSelectedCategory || 'Food';
  ...
  }, [storePicker.visible, storePicker.itemId, storePicker.category, currentPickerCategory, selectedArea, addressCoords]);
  ...
  const combinedStoreList = React.useMemo(() => {
    // 1. Admin Stores defined inside the selected Area document from Admin Panel
    const adminAreaStores = (selectedAreaObj?.stores || []).map((s, idx) => {
      const storeName = typeof s === 'string' ? s : (s.name || '');
      const storeType = typeof s === 'object' ? (s.type || s.category || '') : '';
      return {
        place_id: `admin-area-store-${idx}-${storeName}`,
        placeId: `admin-area-store-${idx}-${storeName}`,
        name: storeName,
        address: `${selectedArea || 'Islamabad'}`,
        rating: '5.0',
        type: storeType,
        category: storeType,
        isGoogleStore: false,
        isBackendStore: true,
        isAdminStore: true,
        isInternational: isInternationalBrandStore(s),
        ...(typeof s === 'object' ? s : {}),
      };
    });

    // 2. Google Places API stores
    const googleList = (googleStores || []).map((g) => ({
      ...
      isInternational: isInternationalBrandStore(g),
      ...g
    }));

    // 3. Backend stores collection
    const backendList = (backendStores || []).map((b) => ({
      ...
      isGoogleStore: false,
      isBackendStore: true,
      isInternational: isInternationalBrandStore(b),
      ...b
    }));
    ...
    const all = Array.from(uniqueMap.values()).filter(s =>
      (s.name || '').toLowerCase().includes(storeSearch.toLowerCase())
    );

    // Sort: International Brands first, Local Brands below
    const internationalList = all.filter(s => isInternationalBrandStore(s));
    const localList = all.filter(s => !isInternationalBrandStore(s));
  ...
  const openStorePicker = (itemId) => {
    ...
    const cat = item?.category || activeSelectedCategory || 'Food';
    setStorePicker({ visible: true, itemId, category: cat });
    setStoreSearch('');
  };
  const closeStorePicker = () => setStorePicker({ visible: false, itemId: null, category: null });
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <Animated.View style={[styles.heroSection, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
          ...
          >
            {HERO_LOOP_CARDS.map((card, index) => {
              const logoSource = card.logoAsset || getBrandLogo(card.title);
              return (
        ...
        {/* ─── CATEGORIES (Informational Showcase) ─── */}
          ...
          <View style={styles.modalContainer}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                const logo = getBrandLogo(storeName);
                return (
                  <TouchableOpacity style={styles.modalItem} onPress={() => pickFeedStore(storeName)}>
                    ...
                    {item.isInternational ? (
                      <View style={[styles.googleBadge, { backgroundColor: '#FEF3C7', borderColor: '#F59E0B', borderWidth: 0.5 }]}>
                        <Text style={[styles.googleBadgeText, { color: '#B45309' }]}>🌐 Int'l Brand</Text>
                      </View>
                    ) : isGoogle ? (
