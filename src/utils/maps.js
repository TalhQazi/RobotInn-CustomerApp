...
const CATEGORY_OSM_TAGS = {
  ...
  'health':          [['amenity', 'pharmacy'], ['amenity', 'clinic'], ['amenity', 'hospital'], ['shop', 'medical_supply']],
  'medical':         [['amenity', 'pharmacy'], ['amenity', 'clinic'], ['shop', 'chemist']],
...
export async function fetchNearbyStoresFromGoogle(category = 'Health', areaName = '', lat = null, lng = null) {
  try {
    // 1. Resolve area coordinates
    let targetLat = lat;
    let targetLng = lng;

    if (!targetLat || !targetLng) {
      const coords = await geocodeAreaNominatim(areaName);
      if (coords) { targetLat = coords.lat; targetLng = coords.lng; }
    }

    // Default to Central Islamabad
    if (!targetLat || !targetLng) {
      targetLat = 33.6844;
      targetLng = 73.0479;
    }

    // 2. Get clean category name
    let categoryName = '';
    if (typeof category === 'object' && category !== null) {
      categoryName = category.name || category.title || category.label || '';
    } else {
    ...
    const cacheKey = `${cleanCatName.toLowerCase()}_${String(areaName).toLowerCase().trim()}_${targetLat.toFixed(2)}_${targetLng.toFixed(2)}`;

    // Return cached stores immediately if available
    if (storeFetchCache.has(cacheKey) && storeFetchCache.get(cacheKey).length > 0) {
      console.log(`[StoreFetch] Returning ${storeFetchCache.get(cacheKey).length} stores from cache for key "${cacheKey}"`);
      return storeFetchCache.get(cacheKey);
    }

    console.log('[StoreFetch] Overpass lookup:', { category: cleanCatName, areaName, targetLat, targetLng });

    // 3. Get OSM tags for this category
    ...
    let tagFilters = osmTags
      .map(([k, v]) => `node["${k}"="${v}"](around:${radiusMeters},${targetLat},${targetLng});`)
      .join('\n');
    ...
    // Fallback: If 0 elements found for specific tag, query general shops in area
    if (elements.length === 0) {
      console.log(`[Overpass] 0 elements for "${cleanCatName}", attempting general shop fallback for area "${areaName}"...`);
      tagFilters = `node["shop"](around:${radiusMeters},${targetLat},${targetLng});\nnode["amenity"="marketplace"](around:${radiusMeters},${targetLat},${targetLng});`;
      ...
      try {
        ...
        console.warn('[Overpass] Fallback error:', fErr?.message);
      }
    }

    console.log(`[Overpass] Found ${elements.length} elements for ${cleanCatName} near ${areaName}`);

    // 5. Map to store objects (only named stores)
    const storeMap = new Map();
    for (const el of elements) {
      const name = el.tags?.name || el.tags?.['name:en'];
      if (!name || storeMap.has(name.toLowerCase())) continue;
      ...
      const address = [
        ...
        el.tags?.['addr:suburb'] || areaName,
      ].filter(Boolean).join(', ') || areaName || '';

      storeMap.set(name.toLowerCase(), {
        place_id: `osm_${el.type}_${el.id}`,
        placeId: `osm_${el.type}_${el.id}`,
        name,
        address,
        rating: el.tags?.stars ? Number(el.tags.stars).toFixed(1) : null,
    ...
    const finalResults = Array.from(storeMap.values()).slice(0, 15);
    ...
    console.warn('[Overpass] Master fetch error:', err?.message || err);
    return [];
  }
}
